...
--------------------------------------------------------------------*/
    ...
    *
    *********************************************************************/
    static void hmi_update_callback_favlist
        (
    ...
    if( action == NAVILITE_POILIST_ACTION_LISTSIZE )
        {
        PRINTF( "\r\n*[NAVILITE-CB NAVILITE_POILIST_ACTION_LISTSIZE] POI LIST ITEM SIZE REQUEST: %d, more data in this request?%d", list_total_items, has_more_items_on_next_request );
        // Integration Note: allocate memory size before receiving the list items.
        }
    else if( action == NAVILITE_POILIST_ACTION_ITEMADD )
        {
        PRINTF( "\r\n[NAVILITE-CB NAVILITE_POILIST_ACTION_ITEMADD]  TBT item add notify for list item no: ", list_item_no );
        if( list_total_items_recevied > list_total_items )
            {
            PRINTF( "\r\nWARNING: the list item count exceeds the item count previously request (NAVILITE_TBTLIST_ACTION_LISTSIZE)! should be less or equal to %d\r\n", list_total_items );
            }
        // Integration Note: replace the following API demo codes for HMI integration
        PRINTF( "* FAV LIST ITEM ADD - list item index:%d progress:(%d/%d)\r\n", list_item_no, list_total_items_recevied, list_total_items );
        PRINTF( " list item index:%d\r\n", list_item->list_item_index );
        PRINTF( " desc size:%d\r\n", list_item->desc_size );
        PRINTF( " dist unit size:%d\r\n", list_item->dist_unit_size );
        PRINTF( " distance * 100:" );
