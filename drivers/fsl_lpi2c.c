...
void LPI2C_MasterSetBaudRate(LPI2C_Type *base, uint32_t sourceClock_Hz, uint32_t baudRate_Hz)
{
    ...
    for (prescale = 1U; prescale <= 128U; prescale = 2U * prescale)
    {
        ...
        for (clkHiCycle = 1U; clkHiCycle < 32U; clkHiCycle++)
        {
            if (clkHiCycle == 1U)
            {
                computedRate = (sourceClock_Hz / prescale) / (1U + 2U + 2U + 2U / prescale);
            }
            else
            {
                computedRate = (sourceClock_Hz / prescale) / (2U * clkHiCycle + 2U + 2U / prescale);
            }
    ...
    {
        value |=
            LPI2C_MCCR0_CLKLO(bestClkHi) | LPI2C_MCCR0_SETHOLD(bestClkHi) | LPI2C_MCCR0_DATAVD(bestClkHi / 2UL);
    }
